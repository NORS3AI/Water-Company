 ...
 *
 * Everything is namespaced behind window.Glacix.
 */
(function () {
	'use strict';

	const SF = (window.Glacix = window.Glacix || {});
	const data = window.GlacixData || {};
	...
	function openDrawer(drawer, overlay) {
		...
		document.body.classList.add('gx-no-scroll');
		// Focus the close button for accessibility.
		const close = qs('[data-gx-drawer-close]', drawer);
	...
	function closeDrawer(drawer, overlay) {
		...
		document.body.classList.remove('gx-no-scroll');
	}
	...
	function initMobileNav() {
		const burger  = qs('[data-gx-burger]');
		const drawer  = qs('[data-gx-drawer="mobile"]');
		const overlay = qs('[data-gx-drawer-overlay]');
		...
		qsa('[data-gx-drawer-close]', drawer).forEach(function (btn) {
	...
	function initCartDrawer() {
		const toggles = qsa('[data-gx-cart-toggle]');
		const drawer  = qs('[data-gx-drawer="cart"]');
		const overlay = qs('[data-gx-drawer-overlay="cart"]');
		...
		qsa('[data-gx-cart-close]', drawer).forEach(function (btn) {
			btn.addEventListener('click', function () {
				closeDrawer(drawer, overlay);
			});
		});
	...
	function initSubmenus() {
		qsa('.gx-nav .menu-item-has-children > a').forEach(function (a) {
